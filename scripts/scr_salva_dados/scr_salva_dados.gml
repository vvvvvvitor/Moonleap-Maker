...
function scr_salva_dados(){
	...
	if(oSaveManager.existe_save_game){
		GooglePlayServices_SavedGames_CommitAndClose("save_1", description, temp_string, path);
	}else{
		GooglePlayServices_SavedGames_CommitNew("save_1", description, temp_string, path);
	}
}