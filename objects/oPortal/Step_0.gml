...
if wait=0
{
	...
	if loadvalue!=undefined {if real(loadvalue)>0  {overlay_index=1}} //open the door
	...
	var loadvalue= variable_struct_get(oSaveManager.struct_main,levelminusoom) //0 =  not complete, >0.5 & <1.0 without bird, >1.0 complete
	/// debug 
	if debug_mode=true {loadvalue=1}
	if secret=true {loadvalue=1}
	///
	if loadvalue!=undefined {if real(loadvalue)>0  {overlay_index=1}}//open the door
	...
	if loadvalue!=undefined { if real(loadvalue)>0 {overlay_index=2} }
	...
	if myloadvalue!= undefined
	{
		...
		if string_length(lvstring)<4	{rank=0} //before dlc
		 
		//if scor=0.5 and scor>!0.5	{rank=0}
		if scor=0 {rank=0}
	}
